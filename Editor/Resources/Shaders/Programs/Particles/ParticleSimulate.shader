Shader "ParticleSimulate"
{
    Compute
    {
        ...
        // age them, and respawn dead ones from the emitter within this frame's spawn budget. Particle state
        // lives in a PERSISTENT storage buffer (survives across frames). Dispatched via DispatchComputeCull so
        // the writes are visible to the billboard VERTEX shader that reads the same buffer.

        LocalSize(64, 1, 1);

        struct Particle
        {
            vec4 PosAge;      // xyz = world position, w = age (seconds)
            vec4 VelLifetime; // xyz = velocity, w = lifetime (<= 0 => dead / unspawned)
        };
        ...
        void main()
        {
            ...
            {
                p.VelLifetime.xyz += u_Gravity.xyz * dt;
                p.PosAge.xyz += p.VelLifetime.xyz * dt;
                p.PosAge.w += dt;
                if ( p.PosAge.w >= p.VelLifetime.w )
                    p.VelLifetime.w = 0.0; // died this frame
            }
            ...
            if ( p.VelLifetime.w <= 0.0 )
            {
                ...
                if ( u_Counts.z == 1u && slot < u_Counts.y )
                {
                    ...
                    // Random direction inside a cone around the emit axis.
                    ...
                    p.PosAge      = vec4( u_EmitterPos.xyz, 0.0 );
                    p.VelLifetime = vec4( dir * speed, max( life, 0.01 ) );
                }
                else
                {
                    p.VelLifetime.w = 0.0; // stay dead
                }
            }
