...
namespace Desert::Editor
{
    ...
    void FileExplorerPanel::DrawNavigationBar()
    {
        ...
        if ( ImGui::Button( ICON_MDI_HOME "##home" ) )
        {
        ...
        if ( ImGui::Button( ICON_MDI_ARROW_RIGHT "##forward" ) )
        {
        ...
        if ( ImGui::Button( ICON_MDI_ARROW_UP "##up" ) )
        {
        ...
        if ( ImGui::Button( ICON_MDI_SYNC "##refresh" ) )
        {
    ...
    void FileExplorerPanel::DrawDirectoryContents()
    {
                                ...
                                ImGuiWindowFlags_AlwaysVerticalScrollbar ) )
        {
            ...
            if ( ImGui::InputTextWithHint( "##search", ICON_MDI_MAGNIFY " Search...", searchFilter,
    ...
    const char* FileExplorerPanel::GetFileIcon( const std::filesystem::path& path )
    {
        static const std::unordered_map<std::string, const char*> iconMap = {
             { ".png", ICON_MDI_FILE_IMAGE },         { ".jpg", ICON_MDI_FILE_IMAGE },
             { ".jpeg", ICON_MDI_FILE_IMAGE },        { ".tga", ICON_MDI_FILE_IMAGE },
             { ".bmp", ICON_MDI_FILE_IMAGE },         { ".h", ICON_MDI_FILE_CODE },
             { ".hpp", ICON_MDI_FILE_CODE },          { ".c", ICON_MDI_FILE_CODE },
             { ".cpp", ICON_MDI_FILE_CODE },          { ".txt", ICON_MDI_FILE_DOCUMENT_ALERT },
             { ".md", ICON_MDI_FILE_DOCUMENT_ALERT }, { ".json", ICON_MDI_FILE_CODE },
             { ".xml", ICON_MDI_FILE_CODE },          { ".yaml", ICON_MDI_FILE_CODE },
             { ".ini", ICON_MDI_FILE_CODE } };

        const auto& ext = path.extension().string();
        auto        it  = iconMap.find( ext );
        return it != iconMap.end() ? it->second : ICON_MDI_FILE;
    }
    ...
    void FileExplorerPanel::DrawEntryContextMenu( const std::filesystem::directory_entry& entry )
    {
        ...
        if ( ImGui::BeginPopupContextItem( popup_id.c_str() ) )
        {
            ...
            if ( !entry.is_directory() && ImGui::MenuItem( ICON_MDI_FOLDER_OPEN " Open in Explorer" ) )
            {
    ...
    void FileExplorerPanel::DrawContextMenu()
    {
        ...
        if ( ImGui::MenuItem(ICON_MDI_FILE_PLUS_OUTLINE " New File" ) )
        {
            CreateNewFile();
        }
        ...
        if ( ImGui::MenuItem( ICON_MDI_SYNC " Refresh" ) )
        {
            RefreshCurrentDirectory();
        }

        if ( ImGui::MenuItem( ICON_MDI_FOLDER_OPEN " Open in Explorer" ) )
        {
            // Platform-specific open in file explorer
        }
    }
