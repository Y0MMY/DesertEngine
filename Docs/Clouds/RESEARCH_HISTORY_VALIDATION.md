...
## 9. Что из §7 подтвердилось измерением, и одна ошибка метода

Измерения — `CALIBRATION.md §HV`. Здесь только то, что относится к ЭТОМУ документу: какие его
предположения выжили.

| §7 говорил | измерение сказало |
|---|---|
| №1 ловила бы пролёт сквозь слой | **подтвердилось.** Ошибка подъёма в слой падает с 1.6022 до 1.4159 при 2 км и до 1.0878 при 4 км |
| №2 в полном виде требует канала, которого нет | **подтвердилось**, и вдобавок: её ВОРОТА канала не требуют, реализуемы и оказались вредны |
| №3 требует полуразрешённой глубины сцены | **подтвердилось.** Её ПОРОГ (относительный, 10 %) отбирает практически те же пиксели, что наш абсолютный: 0 различий на сцене без геометрии, 0.147 % на сцене с ней |
| №4 канала не требует | **подтвердилось**, и стоимость измерена: +0.024 мс из 0.179 |

### ⚠️ ОШИБКА МЕТОДА, ЗАПИСАННАЯ ПОТОМУ, ЧТО ОНА ПОВТОРИМА

§7 не содержал этого предсказания, но работа по нему содержала, и оно было **неверным**. Рассуждение
было такое: гид пишет для пикселя неба `sceneKm = length(farP − camera)`, а `farP` лежит на ПЛОСКОСТИ,
значит расстояние есть `50 км / cos θ` и растёт к краям кадра; репроекция переносит пиксель на другой
θ, поэтому история и этот кадр разойдутся в «расстоянии до сцены» по причине, не имеющей отношения к
сцене. Производная на краю 45-градусного кадра — 0.87 км на градус, при 2°/кадр это 1.7 км против
порога 2 км, при 6°/кадр — 5.2 км, то есть проверка обязана сработать.

**Она не сработала ни при 2°, ни при 6°.** Порог, использованный как линейка (§HV, E1d), показал, что
расхождение на открытом небе не превышает **0.5 км** — вчетверо меньше порога и втрое меньше
предсказанного. Механизм назван верно (расхождение действительно растёт к краям и действительно
исчезает при 0.01 км, где насыщается на 17.2 % пикселей), а **величина была завышена**: угол поворота
между крайними направлениями пути равен 131.5°, а не 180°, и характерный θ по видимой части кадра
заметно меньше углового радиуса.

Вывод для следующего: **арифметика по порядку величины годится, чтобы решить, что мерить, и не годится,
чтобы решить, что сработает.** Проверка стоила один прогон; предсказание, оставленное непроверенным,
стоило бы механизм, встроенный против несуществующего артефакта — ровно то, чем был коэффициент ×1.87.
