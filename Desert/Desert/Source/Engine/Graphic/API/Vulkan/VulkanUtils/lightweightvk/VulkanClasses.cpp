...
//        //                // retrieve source code references - this is very experimental and depends a lot on the
//        //                validation layer output int line = 0; int col = 0; const char* substr1 =
//        //                strstr(cbData->pMessage, "Shader validation error occurred at line "); if (substr1 &&
//        //                sscanf(substr1, "Shader validation error occurred at line %d, column %d.", &line, &col)
//        //                >= 1) {
...
//        //                    if (substr2 && sscanf(substr2, "Shader Module (Shader Module: %[^)])(%p)",
//        //                    shaderModuleDebugName, &shaderModule) == 2) {
//        // #else
//        //                    if (substr2 && sscanf(substr2, "Shader Module (Shader Module: %[^)])(%llu)",
//        //                    shaderModuleDebugName, &shaderModule) == 2) {
//        // #endif // VK_USE_64_BIT_PTR_DEFINES
//        //                        ctx->invokeShaderModuleErrorCallback(line, col, shaderModuleDebugName,
//        //                        shaderModule);
//        //                    }
//        //                    }
//        //                }
...
//                return { VK_FORMAT_D16_UNORM, VK_FORMAT_D16_UNORM_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT,
...
//    }
//
//    bool validateImageLimits( VkImageType imageType, VkSampleCountFlagBits samples, const VkExtent3D& extent,
//                              const VkPhysicalDeviceLimits& limits, lvk::Result* outResult )
//    {
//        using lvk::Result;
//
//        if ( samples != VK_SAMPLE_COUNT_1_BIT && !LVK_VERIFY( imageType == VK_IMAGE_TYPE_2D ) )
//        {
//            Result::setResult( outResult, Result( Result::Code::ArgumentOutOfRange,
//                                                  "Multisampling is supported only for 2D images" ) );
//            return false;
//        }
//
//        if ( imageType == VK_IMAGE_TYPE_2D && !LVK_VERIFY( extent.width <= limits.maxImageDimension2D &&
...
//            return false;
//        }
//        if ( imageType == VK_IMAGE_TYPE_3D && !LVK_VERIFY( extent.width <= limits.maxImageDimension3D &&
...
//    {
//        if ( !LVK_VERIFY( range.dimensions.width > 0 && range.dimensions.height > 0 ||
//                          range.dimensions.depth > 0 || range.numLayers > 0 || range.numMipLevels > 0 ) )
//        {
//            return lvk::Result{ lvk::Result::Code::ArgumentOutOfRange,
//                                "width, height, depth numLayers, and numMipLevels must be > 0" };
//        }
//        if ( range.mipLevel > numLevels )
//        {
//            return lvk::Result{ lvk::Result::Code::ArgumentOutOfRange,
//                                "range.mipLevel exceeds texture mip-levels" };
//        }
//
...
//        {
//            return lvk::Result{ lvk::Result::Code::ArgumentOutOfRange,
//                                "range dimensions exceed texture dimensions" };
//        }
...
//    }
//
//    void getDeviceExtensionProps( VkPhysicalDevice dev, std::vector<VkExtensionProperties>& props,
...
//        colorTex->transitionLayout( buffer, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
//                                    VkImageSubresourceRange{ VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS,
...
//
//        LOG_INFO(
//             "Could not find a native swap chain format that matched our designed swapchain format. Defaulting "
//             "to first supported format." );
//
//        return formats[0];
//    }
//
...
//}
//
//void lvk::VulkanBuffer::invalidateMappedMemory( const VulkanContext& ctx, VkDeviceSize offset,
...
//    if ( LVK_VULKAN_USE_VMA )
//    {
//        vmaInvalidateAllocation( static_cast<VmaAllocator>( ctx.getVmaAllocator() ), vmaAllocation_, offset,
//                                 size );
//    }
//    else
//    {
...
//}
//
//VkImageView lvk::VulkanImage::createImageView( VkDevice device, VkImageViewType type, VkFormat format,
//                                               VkImageAspectFlags aspectMask, uint32_t baseLevel,
//                                               uint32_t numLevels, uint32_t baseLayer, uint32_t numLayers,
...
//}
//
//void lvk::VulkanImage::transitionLayout( VkCommandBuffer commandBuffer, VkImageLayout newImageLayout,
...
//              ? ( isDepthAttachment() ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
//                                      : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL )
//              : vkImageLayout_;
//
//    if ( newImageLayout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL )
//    {
//        newImageLayout = isDepthAttachment() ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
//                                             : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
//    }
//
...
//             ( vkFormatProperties_.optimalTilingFeatures & formatFeatureMask ) == formatFeatureMask;
//
//        if ( !hardwareDownscalingSupported )
//        {
//            LOG_WARN( "Doesn't support hardware downscaling of this image format: %p", vkImageFormat_ );
//            return;
//        }
//    }
//
...
//       vkFormatProperties_.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT );
//
...
//        {
//            // 1: Transition the i-th level to VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; it will be copied into from
//            // the (i-1)-th layer
//            lvk::imageMemoryBarrier2(
//                 commandBuffer, vkImage_,
...
//            };
//
//            // 2: Blit the image from the prev mip-level (i-1) (VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) to the
//            // current mip-level (i) (VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
//#if LVK_VULKAN_PRINT_COMMANDS
//            LOG_INFO( "%p vkCmdBlitImage()\n", commandBuffer );
...
//            };
//            vkCmdBlitImage( commandBuffer, vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, vkImage_,
//                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, blitFilter );
//            // 3: Transition i-th level to VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL as it will be read from in the next
//            // iteration
//            lvk::imageMemoryBarrier2(
//                 commandBuffer, vkImage_,
//                 StageAccess{ .stage  = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
...
//}
//
//VkImageView lvk::VulkanImage::getOrCreateVkImageViewForFramebuffer( VulkanContext& ctx, uint8_t level,
//                                                                    uint16_t layer )
//{
...
//              "Image View: '%s' imageViewForFramebuffer_[%u][%u]", debugName_, level, layer );
//
//    imageViewForFramebuffer_[level][layer] =
//         createImageView( ctx.getVkDevice(), VK_IMAGE_VIEW_TYPE_2D, vkImageFormat_, getImageAspectFlags(), level,
//                          1u, layer, 1u, {}, nullptr, debugNameImageView );
//
//    return imageViewForFramebuffer_[level][layer];
//}
//
//lvk::VulkanSwapchain::VulkanSwapchain( VulkanContext& ctx, uint32_t width, uint32_t height )
//     : ctx_( ctx ), device_( ctx.vkDevice_ ), graphicsQueue_( ctx.deviceQueues_.graphicsQueue ), width_( width ),
//       height_( height )
//{
...
//    LVK_ASSERT_MSG( queueFamilySupportsPresentation == VK_TRUE,
...
//         ( ctx.deviceSurfaceCaps_.supportedCompositeAlpha & VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR ) != 0;
...
//
//        VK_ASSERT(
//             lvk::setDebugObjectName( device_, VK_OBJECT_TYPE_IMAGE, (uint64_t)image.vkImage_, debugNameImage ) );
//
//        image.imageView_ = image.createImageView( device_, VK_IMAGE_VIEW_TYPE_2D, surfaceFormat_.format,
...
//        VkResult    r = vkAcquireNextImageKHR( device_, swapchain_, UINT64_MAX, acquireSemaphore, VK_NULL_HANDLE,
//                                               &currentImageIndex_ );
...
//}
//
//lvk::VulkanImmediateCommands::VulkanImmediateCommands( VkDevice device, uint32_t queueFamilyIndex,
...
//    {
//        LOG_INFO( "Waiting for command buffers...\n" );
...
//    {
//        // we are waiting for a buffer which has not been submitted - this is probably a logic error somewhere in
//        // the calling code
//        return;
//    }
//
...
//    {
//        // do not ask the Vulkan API about it, just let it retire naturally (when submitId for this bufferIndex
//        // gets incremented)
//        return false;
//    }
//
...
//lvk::VulkanPipelineBuilder::vertexInputState( const VkPipelineVertexInputStateCreateInfo& state )
//{
...
//
//lvk::VulkanPipelineBuilder&
//lvk::VulkanPipelineBuilder::colorAttachments( const VkPipelineColorBlendAttachmentState* states,
...
//         depthStencilState_.stencilTestEnable == VK_TRUE || failOp != VK_STENCIL_OP_KEEP ||
...
//}
//
//VkResult lvk::VulkanPipelineBuilder::build( VkDevice device, VkPipelineCache pipelineCache,
...
//        ctx_->bindDefaultDescriptorSets( wrapper_->cmdBuf_, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
...
//         .pNext      = nullptr,
//         .pLabelName = label,
//         .color      = { float( ( colorRGBA >> 0 ) & 0xff ) / 255.0f, float( ( colorRGBA >> 8 ) & 0xff ) / 255.0f,
//                         float( ( colorRGBA >> 16 ) & 0xff ) / 255.0f, float( ( colorRGBA >> 24 ) & 0xff ) / 255.0f },
//    };
...
//                          tex.isStorageImage() ? VK_IMAGE_LAYOUT_GENERAL
//                                               : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
//                          VkImageSubresourceRange{ tex.getImageAspectFlags(), 0, VK_REMAINING_MIP_LEVELS, 0,
//                                                   VK_REMAINING_ARRAY_LAYERS } );
//}
//
//void lvk::CommandBuffer::bufferBarrier( BufferHandle handle, VkPipelineStageFlags2 srcStage,
...
//        LVK_ASSERT( buf );
//        if ( ( buf->vkUsageFlags_ & VK_BUFFER_USAGE_INDEX_BUFFER_BIT ) ||
//             ( buf->vkUsageFlags_ & VK_BUFFER_USAGE_VERTEX_BUFFER_BIT ) )
//        {
...
//             wrapper_->cmdBuf_, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
//             VkImageSubresourceRange{ flags, 0, VK_REMAINING_MIP_LEVELS, 0, VK_REMAINING_ARRAY_LAYERS } );
//    }
//    // handle depth MSAA
...
//             .clearValue         = { .color = { .float32 = { descColor.clearColor[0], descColor.clearColor[1],
//                                                             descColor.clearColor[2], descColor.clearColor[3] } } },
//        };
//        // handle MSAA
//        if ( descColor.storeOp == StoreOp_MsaaResolve )
//        {
//            LVK_ASSERT( samples > 1 );
//            LVK_ASSERT_MSG( !attachment.resolveTexture.empty(),
...
//            colorAttachments[i].resolveImageView  = colorResolveTexture.getOrCreateVkImageViewForFramebuffer(
//                 *ctx_, descColor.level, descColor.layer );
...
//        LVK_ASSERT_MSG( descDepth.level == mipLevel,
...
//             .clearValue         = { .depthStencil = { .depth   = descDepth.clearDepth,
//                                                       .stencil = descDepth.clearStencil } },
//        };
//        // handle depth MSAA
...
//            depthAttachment.resolveImageView      = depthResolveTexture.getOrCreateVkImageViewForFramebuffer(
//                 *ctx_, descDepth.level, descDepth.layer );
...
//    {
//        LVK_ASSERT( false );
//        LOG_WARN( "Make sure your render pass and render pipeline both have matching depth attachments" );
//    }
//
//    VkPipeline pipeline = ctx_->getVkPipeline( handle );
//
...
//}
//
//void lvk::CommandBuffer::cmdBindIndexBuffer( BufferHandle indexBuffer, IndexFormat indexFormat,
...
//    {
//        LOG_WARN( "Push constants size exceeded %u (max %u bytes)", size + offset, limits.maxPushConstantsSize );
//    }
//
//    if ( currentPipelineGraphics_.empty() && currentPipelineCompute_.empty() &&
//         currentPipelineRayTracing_.empty() )
//    {
...
//}
//
//void lvk::CommandBuffer::cmdDraw( uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
...
//}
//
//void lvk::CommandBuffer::cmdDrawIndexed( uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex,
...
//}
//
//void lvk::CommandBuffer::cmdDrawIndirect( BufferHandle indirectBuffer, size_t indirectBufferOffset,
...
//
//    vkCmdDrawIndirect( wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset, drawCount,
//                       stride ? stride : sizeof( VkDrawIndirectCommand ) );
//}
//
//void lvk::CommandBuffer::cmdDrawIndexedIndirect( BufferHandle indirectBuffer, size_t indirectBufferOffset,
//                                                 uint32_t drawCount, uint32_t stride )
//{
//    LVK_PROFILER_FUNCTION();
...
//    LVK_ASSERT( bufIndirect );
//
//    vkCmdDrawIndexedIndirect( wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset, drawCount,
//                              stride ? stride : sizeof( VkDrawIndexedIndirectCommand ) );
//}
//
//void lvk::CommandBuffer::cmdDrawIndexedIndirectCount( BufferHandle indirectBuffer, size_t indirectBufferOffset,
//                                                      BufferHandle countBuffer, size_t countBufferOffset,
...
//    LVK_ASSERT( bufCount );
//
//    vkCmdDrawIndexedIndirectCount( wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset,
//                                   bufCount->vkBuffer_, countBufferOffset, maxDrawCount,
//                                   stride ? stride : sizeof( VkDrawIndexedIndirectCommand ) );
//}
//
...
//
//    vkCmdDrawMeshTasksEXT( wrapper_->cmdBuf_, threadgroupCount.width, threadgroupCount.height,
//                           threadgroupCount.depth );
//}
//
//void lvk::CommandBuffer::cmdDrawMeshTasksIndirect( BufferHandle indirectBuffer, size_t indirectBufferOffset,
//                                                   uint32_t drawCount, uint32_t stride )
//{
//    LVK_PROFILER_FUNCTION();
...
//    LVK_ASSERT( bufIndirect );
//
//    vkCmdDrawMeshTasksIndirectEXT( wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset, drawCount,
//                                   stride ? stride : sizeof( VkDrawMeshTasksIndirectCommandEXT ) );
//}
//
//void lvk::CommandBuffer::cmdDrawMeshTasksIndirectCount( BufferHandle indirectBuffer, size_t indirectBufferOffset,
//                                                        BufferHandle countBuffer, size_t countBufferOffset,
//                                                        uint32_t maxDrawCount, uint32_t stride )
//{
//    LVK_PROFILER_FUNCTION();
//    LVK_PROFILER_GPU_ZONE( "cmdDrawMeshTasksIndirectCount()", ctx_, wrapper_->cmdBuf_,
//                           LVK_PROFILER_COLOR_CMD_DRAW );
//
//    lvk::VulkanBuffer* bufIndirect = ctx_->buffersPool_.get( indirectBuffer );
//    lvk::VulkanBuffer* bufCount    = ctx_->buffersPool_.get( countBuffer );
//
...
//    LVK_ASSERT( bufCount );
//
//    vkCmdDrawMeshTasksIndirectCountEXT( wrapper_->cmdBuf_, bufIndirect->vkBuffer_, indirectBufferOffset,
//                                        bufCount->vkBuffer_, countBufferOffset, maxDrawCount,
//                                        stride ? stride : sizeof( VkDrawMeshTasksIndirectCommandEXT ) );
//}
//
...
//    }
//
//    vkCmdTraceRaysKHR( wrapper_->cmdBuf_, &rtps->sbtEntryRayGen, &rtps->sbtEntryMiss, &rtps->sbtEntryHit,
...
//}
//
//void lvk::CommandBuffer::cmdClearColorImage( TextureHandle tex, const ClearColorValue& value,
...
//
//    lvk::imageMemoryBarrier2(
//         wrapper_->cmdBuf_, img->vkImage_,
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         StageAccess{ .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_WRITE_BIT },
//         img->vkImageLayout_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, range );
//
//    vkCmdClearColorImage( wrapper_->cmdBuf_, img->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
//                          reinterpret_cast<const VkClearColorValue*>( &value ), 1, &range );
//
//    // a ternary cascade...
...
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, newLayout, range );
//
//    img->vkImageLayout_ = newLayout;
//}
//
//void lvk::CommandBuffer::cmdCopyImage( TextureHandle src, TextureHandle dst, const Dimensions& extent,
//                                       const Offset3D& srcOffset, const Offset3D& dstOffset,
//                                       const TextureLayers& srcLayers, const TextureLayers& dstLayers )
//{
...
//    const bool       coversFullDstImage = dstExtent.width == extent.width && dstExtent.height == extent.height &&
//                                    dstExtent.depth == extent.depth && dstOffset.x == 0 && dstOffset.y == 0 &&
//                                    dstOffset.z == 0;
//
//    LVK_ASSERT( coversFullDstImage || imgDst->vkImageLayout_ != VK_IMAGE_LAYOUT_UNDEFINED );
//
//    lvk::imageMemoryBarrier2(
//         wrapper_->cmdBuf_, imgSrc->vkImage_,
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         StageAccess{ .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_READ_BIT },
//         imgSrc->vkImageLayout_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, rangeSrc );
//    lvk::imageMemoryBarrier2(
//         wrapper_->cmdBuf_, imgDst->vkImage_,
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         StageAccess{ .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_WRITE_BIT },
...
//         getBytesPerPixel( imgSrc->vkImageFormat_ ) == getBytesPerPixel( imgDst->vkImageFormat_ );
//
//    isCompatible ? vkCmdCopyImage( wrapper_->cmdBuf_, imgSrc->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
//                                   imgDst->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &regionCopy )
//                 : vkCmdBlitImage( wrapper_->cmdBuf_, imgSrc->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
//                                   imgDst->vkImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &regionBlit,
//                                   VK_FILTER_LINEAR );
//
//    lvk::imageMemoryBarrier2(
//         wrapper_->cmdBuf_, imgSrc->vkImage_,
...
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, imgSrc->vkImageLayout_, rangeSrc );
//
//    // a ternary cascade...
...
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, newLayout, rangeSrc );
//
...
//    };
//    vkGetAccelerationStructureBuildSizesKHR( ctx_->getVkDevice(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
//                                             &accelerationStructureBuildGeometryInfo,
//                                             &as->buildRangeInfo.primitiveCount,
//                                             &accelerationStructureBuildSizesInfo );
//
//    if ( !as->scratchBuffer.valid() ||
//         getBufferSize( ctx_, as->scratchBuffer ) < accelerationStructureBuildSizesInfo.buildScratchSize )
//    {
...
//}
//
//void lvk::VulkanStagingDevice::bufferSubData( VulkanBuffer& buffer, size_t dstOffset, size_t size,
//                                              const void* data )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//        }
//        vkCmdPipelineBarrier( wrapper.cmdBuf_, VK_PIPELINE_STAGE_TRANSFER_BIT, dstMask, VkDependencyFlags{}, 0,
...
//}
//
//void lvk::VulkanStagingDevice::imageData2D( VulkanImage& image, const VkRect2D& imageRegion, uint32_t baseMipLevel,
//                                            uint32_t numMipLevels, uint32_t layer, uint32_t numLayers,
...
//
//    LVK_ASSERT_MSG(
//         !imageRegion.offset.x && !imageRegion.offset.y && imageRegion.extent.width == width &&
...
//             lvk::getTextureBytesPerLayer( image.vkExtent_.width, image.vkExtent_.height, texFormat, i );
...
//    // No support for copying image in multiple smaller chunk sizes. If we get smaller buffer size than
//    // storageSize, we will wait for GPU idle and get bigger chunk.
...
//            lvk::imageMemoryBarrier2(
//                 wrapper.cmdBuf_, image.vkImage_,
//                 StageAccess{ .stage = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT, .access = VK_ACCESS_2_NONE },
//                 StageAccess{ .stage  = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
//                              .access = VK_ACCESS_2_TRANSFER_WRITE_BIT },
...
//
//#if LVK_VULKAN_PRINT_COMMANDS
//            LOG_INFO( "%p vkCmdCopyBufferToImage()\n", wrapper.cmdBuf_ );
//#endif // LVK_VULKAN_PRINT_COMMANDS
...
//                     .bufferImageHeight = 0,
//                     .imageSubresource =
//                          VkImageSubresourceLayers{ numPlanes > 1 ? VK_IMAGE_ASPECT_PLANE_0_BIT << plane
//                                                                  : imageAspect,
//                                                    currentMipLevel, layer, 1 },
...
//                };
//                vkCmdCopyBufferToImage( wrapper.cmdBuf_, stagingBuffer->vkBuffer_, image.vkImage_,
//                                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy );
//                planeOffset += lvk::getTextureBytesPerPlane( imageRegion.extent.width, imageRegion.extent.height,
//                                                             vkFormatToFormat( format ), plane );
//            }
//
...
//                 VkImageSubresourceRange{ imageAspect, currentMipLevel, 1, layer, 1 } );
//
//            offset += lvk::getTextureBytesPerLayer( imageRegion.extent.width, imageRegion.extent.height, texFormat,
//                                                    currentMipLevel );
//        }
//    }
//
...
//}
//
//void lvk::VulkanStagingDevice::imageData3D( VulkanImage& image, const VkOffset3D& offset, const VkExtent3D& extent,
//                                            VkFormat format, const void* data )
//{
//    LVK_PROFILER_FUNCTION();
//    LVK_ASSERT_MSG( image.numLevels_ == 1, "Can handle only 3D images with exactly 1 mip-level" );
//    LVK_ASSERT_MSG( ( offset.x == 0 ) && ( offset.y == 0 ) && ( offset.z == 0 ),
...
//    ensureStagingBufferSize( storageSize );
//
//    LVK_ASSERT_MSG( storageSize <= stagingBufferSize_,
...
//}
//
//void lvk::VulkanStagingDevice::getImageData( VulkanImage& image, const VkOffset3D& offset,
//                                             const VkExtent3D& extent, VkImageSubresourceRange range,
//                                             VkFormat format, void* outData )
//{
...
//    lvk::imageMemoryBarrier2(
//         wrapper1.cmdBuf_, image.vkImage_,
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         StageAccess{ .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_READ_BIT },
...
//         .imageExtent = extent,
//    };
//    vkCmdCopyImageToBuffer( wrapper1.cmdBuf_, image.vkImage_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
//                            stagingBuffer->vkBuffer_, 1, &copy );
//
...
//
//    lvk::imageMemoryBarrier2(
//         wrapper2.cmdBuf_, image.vkImage_,
//         StageAccess{ .stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_READ_BIT },
//         StageAccess{ .stage  = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT,
//                      .access = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT },
//         VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image.vkImageLayout_, range );
//
...
//
//    // if the combined size of the new staging buffer and the existing one is larger than the limit imposed by some
//    // architectures on buffers that are device and host visible, we need to wait for the current buffer to be
...
//                                          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, nullptr, debugName ) };
...
//
//    // if we can't find an available region that is big enough to store requestedAlignedSize, return whatever we
//    // could find, which will be stored in bestNextIt
...
//    waitAndReset();
//
//    // waitAndReset() adds a region that spans the entire buffer. Since we'll be using part of it, we need to
//    // replace it with a used block and an unused portion
//    regions_.clear();
//
...
//}
//
//lvk::VulkanContext::VulkanContext( const lvk::ContextConfig& config, void* window, void* display,
...
//    {
//        LOG_WARN( "volkInitialize() failed\n" );
//        exit( 255 );
//    };
//
...
//
//    LOG_INFO( "Vulkan graphics pipelines created: %u\n", VulkanPipelineBuilder::getNumPipelinesCreated() );
//}
//
...
//
//        tex.transitionLayout( vkCmdBuffer->wrapper_->cmdBuf_, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
//                              VkImageSubresourceRange{ VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 0,
//                                                       VK_REMAINING_ARRAY_LAYERS } );
//    }
//
...
//                                                                 const char* debugName, Result* outResult )
//{
...
//}
//
//lvk::Holder<lvk::QueryPoolHandle> lvk::VulkanContext::createQueryPool( uint32_t numQueries, const char* debugName,
//                                                                       Result* outResult )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//}
//
//lvk::Holder<lvk::AccelStructHandle> lvk::VulkanContext::createAccelerationStructure( const AccelStructDesc& desc,
//                                                                                     Result* outResult )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//                           Result( Result::Code::RuntimeError, "VK_KHR_acceleration_structure is not enabled" ) );
//        return {};
//    }
//
//    Result result;
...
//            Result::setResult( outResult,
//                               Result( Result::Code::ArgumentOutOfRange, "Invalid acceleration structure type" ) );
//            return {};
//    }
//
...
//    {
//        Result::setResult( outResult,
//                           Result( Result::Code::RuntimeError, "Cannot create AccelerationStructure" ) );
//        return {};
//    }
//
//    Result::setResult( outResult, result );
...
//}
//
//lvk::Holder<lvk::SamplerHandle> lvk::VulkanContext::createSampler( const SamplerStateDesc& desc,
//                                                                   Result*                 outResult )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//         samplerStateDescToVkSamplerCreateInfo( desc, getVkPhysicalDeviceProperties().limits );
//
...
//        Result::setResult( outResult, Result::Code::ArgumentOutOfRange,
//                           "Multisampled 3D images are not supported" );
//        return {};
//    }
//
//    if ( !LVK_VERIFY( desc.numMipLevels <=
...
//
//    if ( !LVK_VERIFY( validateImageLimits( vkImageType, vkSamples, vkExtent,
//                                           getVkPhysicalDeviceProperties().limits, outResult ) ) )
//    {
//        return {};
//    }
//
...
//        vkCreateFlags |=
//             VK_IMAGE_CREATE_DISJOINT_BIT | VK_IMAGE_CREATE_ALIAS_BIT | VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT;
...
//        };
//
//        VkResult result = vmaCreateImage( (VmaAllocator)getVmaAllocator(), &ci, &vmaAllocInfo, &image.vkImage_,
//                                          &image.vmaAllocation_, nullptr );
//
//        if ( !LVK_VERIFY( result == VK_SUCCESS ) )
//        {
//            LOG_WARN( "Failed: error result: %d, memflags: %d,  imageformat: %d\n", result, memFlags,
...
//               .image = img },
//             { .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_REQUIREMENTS_INFO_2,
//               .pNext = numPlanes > 1 ? &planes[1] : nullptr,
//               .image = img },
//             { .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_REQUIREMENTS_INFO_2,
//               .pNext = numPlanes > 2 ? &planes[2] : nullptr,
//               .image = img },
//        };
//
...
//            VK_ASSERT( lvk::allocateMemory2( vkPhysicalDevice_, vkDevice_, &memRequirements[p], memFlags,
//                                             &image.vkMemory_[p] ) );
//        }
//
...
//             lvk::getBindImageMemoryInfo( isDisjoint ? &bindImagePlaneMemoryInfo[0] : nullptr, img,
//                                          image.vkMemory_[0] ),
...
//
//    VK_ASSERT(
//         lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_IMAGE, (uint64_t)image.vkImage_, debugNameImage ) );
//
...
//         isDisjoint ? getOrCreateYcbcrConversionInfo( desc.format ) : nullptr;
//
//    image.imageView_ =
//         image.createImageView( vkDevice_, vkImageViewType, vkFormat, aspect, 0, VK_REMAINING_MIP_LEVELS, 0,
//                                numLayers, mapping, ycbcrInfo, debugNameImageView );
//
...
//            image.imageViewStorage_ =
//                 image.createImageView( vkDevice_, vkImageViewType, vkFormat, aspect, 0, VK_REMAINING_MIP_LEVELS,
//                                        0, numLayers, {}, ycbcrInfo, debugNameImageView );
...
//        Result         res       = upload(
//             handle,
//             { .dimensions = desc.dimensions, .numLayers = numLayers, .numMipLevels = desc.dataNumMipLevels },
//             desc.data );
//        if ( !res.isOk() )
//        {
...
//
//    image.imageView_ =
//         image.createImageView( vkDevice_, vkImageViewType, image.vkImageFormat_, aspect, desc.mipLevel,
//                                desc.numMipLevels, desc.layer, desc.numLayers, mapping, nullptr, debugName );
//
//    if ( !LVK_VERIFY( image.imageView_ != VK_NULL_HANDLE ) )
//    {
//        Result::setResult( outResult, Result::Code::RuntimeError, "Cannot create VkImageView" );
...
//            image.imageViewStorage_ =
//                 image.createImageView( vkDevice_, vkImageViewType, image.vkImageFormat_, aspect, desc.mipLevel,
//                                        desc.numMipLevels, desc.layer, desc.numLayers, {}, nullptr, debugName );
//            LVK_ASSERT( image.imageViewStorage_ != VK_NULL_HANDLE );
//        }
//    }
//
...
//
//    LVK_ASSERT( buffersPool_.get( desc.indexBuffer )->vkUsageFlags_ &
//                VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
//    LVK_ASSERT( buffersPool_.get( desc.vertexBuffer )->vkUsageFlags_ &
//                VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
//    LVK_ASSERT( buffersPool_.get( desc.transformBuffer )->vkUsageFlags_ &
//                VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
//
...
//                             .vertexStride  = desc.vertexStride ? desc.vertexStride
//                                                                : lvk::getVertexFormatSize( desc.vertexFormat ),
...
//         vkDevice_, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &accelerationStructureBuildGeometryInfo,
//         &desc.buildRange.primitiveCount, &accelerationStructureBuildSizesInfo );
...
//
//    const VkAccelerationStructureBuildRangeInfoKHR* accelerationBuildStructureRangeInfos[] = {
//         &accelStruct.buildRangeInfo };
//
//    lvk::ICommandBuffer& buffer = acquireCommandBuffer();
//    vkCmdBuildAccelerationStructuresKHR( lvk::getVkCommandBuffer( buffer ), 1, &accelerationBuildGeometryInfo,
//                                         accelerationBuildStructureRangeInfos );
//    wait( submit( buffer, {} ) );
//
...
//         vkGetAccelerationStructureDeviceAddressKHR( vkDevice_, &accelerationDeviceAddressInfo );
//
...
//    LVK_ASSERT( desc.buildRange.primitiveCount );
//    LVK_ASSERT( buffersPool_.get( desc.instancesBuffer )->vkUsageFlags_ &
//                VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR );
//
//    const VkAccelerationStructureGeometryKHR accelerationStructureGeometry{
//         .sType        = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR,
...
//}
//
//static_assert( 1 << ( sizeof( lvk::Format ) * 8 ) <=
//                    LVK_ARRAY_NUM_ELEMENTS( lvk::VulkanContextImpl::ycbcrConversionData_ ),
...
//
//    const VkSamplerCreateInfo cinfo =
//         samplerStateDescToVkSamplerCreateInfo( {}, getVkPhysicalDeviceProperties().limits );
//
//    pimpl_->ycbcrConversionData_[format].info    = info;
//    pimpl_->ycbcrConversionData_[format].sampler = {
//         this, this->createSampler( cinfo, nullptr, format, "YUV sampler" ) };
...
//    {
//        LVK_ASSERT_MSG( tescModule && teseModule,
...
//        {
//            LOG_WARN( "Push constants size exceeded %u (max %u bytes)", pushConstantsSize,
...
//                      rps->desc_.debugName );
//        }
//        VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)layout,
//                                            pipelineLayoutName ) );
//    }
//
//    lvk::VulkanPipelineBuilder()
...
//         .rasterizationSamples( getVulkanSampleCountFlags( desc.samplesCount, getFramebufferMSAABitMask() ),
...
//         .stencilMasks( VK_STENCIL_FACE_FRONT_BIT, 0xFF, desc.frontFaceStencil.writeMask,
//                        desc.frontFaceStencil.readMask )
//         .stencilMasks( VK_STENCIL_FACE_BACK_BIT, 0xFF, desc.backFaceStencil.writeMask,
...
//                                         VK_SHADER_STAGE_MESH_BIT_EXT, meshModule->sm, desc.entryPointMesh, &si )
//                                  : lvk::getPipelineShaderStageCreateInfo(
//                                         VK_SHADER_STAGE_VERTEX_BIT, vertModule->sm, desc.entryPointVert, &si ) )
//         .shaderStage( lvk::getPipelineShaderStageCreateInfo( VK_SHADER_STAGE_FRAGMENT_BIT, fragModule->sm,
//                                                              desc.entryPointFrag, &si ) )
//         .shaderStage( tescModule
//                            ? lvk::getPipelineShaderStageCreateInfo( VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
//                                                                     tescModule->sm, desc.entryPointTesc, &si )
//                            : VkPipelineShaderStageCreateInfo{ .module = VK_NULL_HANDLE } )
//         .shaderStage( teseModule
//                            ? lvk::getPipelineShaderStageCreateInfo( VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT,
//                                                                     teseModule->sm, desc.entryPointTese, &si )
//                            : VkPipelineShaderStageCreateInfo{ .module = VK_NULL_HANDLE } )
//         .shaderStage( geomModule ? lvk::getPipelineShaderStageCreateInfo(
//                                         VK_SHADER_STAGE_GEOMETRY_BIT, geomModule->sm, desc.entryPointGeom, &si )
//                                  : VkPipelineShaderStageCreateInfo{ .module = VK_NULL_HANDLE } )
...
//    VkSpecializationMapEntry entries[SpecializationConstantDesc::LVK_SPECIALIZATION_CONSTANTS_MAX] = {};
//
//    const VkSpecializationInfo siComp =
//         lvk::getPipelineShaderStageSpecializationInfo( rtps->desc_.specInfo, entries );
//
...
//    if ( shaderModule )                                                                                           \
//        ciShaderStages[numShaderStages++] =                                                                       \
//             lvk::getPipelineShaderStageCreateInfo( vkStageFlag, shaderModule->sm, "main", &siComp );
...
//    };
//    VK_ASSERT( vkCreateRayTracingPipelinesKHR( vkDevice_, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &ciRayTracingPipeline,
//                                               nullptr, &rtps->pipeline_ ) );
//
...
//         getAlignedSize( props.shaderGroupHandleSize, props.shaderGroupHandleAlignment );
//    const uint32_t sbtSize = numShaderGroups * handleSizeAligned;
//
//    LVK_ASSERT( sbtSize );
//
//    std::vector<uint8_t> shaderHandleStorage( sbtSize );
//    VK_ASSERT( vkGetRayTracingShaderGroupHandlesKHR( vkDevice_, rtps->pipeline_, 0, numShaderGroups, sbtSize,
...
//    {
//        memcpy( sbtStorage.data() + i * sbtEntrySizeAligned, shaderHandleStorage.data() + i * handleSizeAligned,
//                handleSize );
//    }
//
...
//
//        const VkSpecializationInfo siComp =
//             lvk::getPipelineShaderStageSpecializationInfo( cps->desc_.specInfo, entries );
//
//        // create pipeline layout
//        {
//            // duplicate for MoltenVK
...
//            }
//            VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_PIPELINE_LAYOUT,
//                                                (uint64_t)cps->pipelineLayout_, pipelineLayoutName ) );
//        }
//
//        const VkComputePipelineCreateInfo ci = {
//             .sType              = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
//             .flags              = 0,
//             .stage              = lvk::getPipelineShaderStageCreateInfo( VK_SHADER_STAGE_COMPUTE_BIT, sm->sm,
//                                                                          cps->desc_.entryPoint, &siComp ),
//             .layout             = cps->pipelineLayout_,
//             .basePipelineHandle = VK_NULL_HANDLE,
//             .basePipelineIndex  = -1,
//        };
//        VK_ASSERT( vkCreateComputePipelines( vkDevice_, pipelineCache_, 1, &ci, nullptr, &cps->pipeline_ ) );
//        VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_PIPELINE, (uint64_t)cps->pipeline_,
//                                            cps->desc_.debugName ) );
//    }
//
...
//}
//
//lvk::Holder<lvk::ComputePipelineHandle> lvk::VulkanContext::createComputePipeline( const ComputePipelineDesc& desc,
//                                                                                   Result* outResult )
//{
...
//}
//
//lvk::Holder<lvk::RayTracingPipelineHandle>
//lvk::VulkanContext::createRayTracingPipeline( const RayTracingPipelineDesc& desc, Result* outResult )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//    {
//        Result::setResult( outResult,
//                           Result( Result::Code::RuntimeError, "VK_KHR_ray_tracing_pipeline is not enabled" ) );
//        return {};
//    }
//
...
//}
//
//lvk::Holder<lvk::RenderPipelineHandle> lvk::VulkanContext::createRenderPipeline( const RenderPipelineDesc& desc,
//                                                                                 Result* outResult )
//{
...
//        {
//            Result::setResult( outResult, Result::Code::ArgumentOutOfRange,
//                               "Cannot have vertexInput with mesh shaders" );
//            return {};
//        }
//        if ( !LVK_VERIFY( !desc.smVert.valid() ) )
//        {
//            Result::setResult( outResult, Result::Code::ArgumentOutOfRange,
//                               "Cannot have both vertex and mesh shaders" );
//            return {};
//        }
//        if ( !LVK_VERIFY( !desc.smTesc.valid() && !desc.smTese.valid() ) )
//        {
//            Result::setResult( outResult, Result::Code::ArgumentOutOfRange,
//                               "Cannot have both tessellation and mesh shaders" );
//            return {};
//        }
//        if ( !LVK_VERIFY( !desc.smGeom.valid() ) )
//        {
//            Result::setResult( outResult, Result::Code::ArgumentOutOfRange,
//                               "Cannot have both geometry and mesh shaders" );
//            return {};
//        }
//    }
//    else
...
//             [vma = getVmaAllocator(), buffer = buf->vkBuffer_, allocation = buf->vmaAllocation_]()
...
//        }
//        deferredTask( std::packaged_task<void()>(
//             [device = vkDevice_, buffer = buf->vkBuffer_, memory = buf->vkMemory_]()
//             {
...
//            {
//                deferredTask(
//                     std::packaged_task<void()>( [device = getVkDevice(), imageView = v]()
//                                                 { vkDestroyImageView( device, imageView, nullptr ); } ) );
//            }
//        }
//    }
...
//        }
//        deferredTask( std::packaged_task<void()>(
//             [vma = getVmaAllocator(), image = tex->vkImage_, allocation = tex->vmaAllocation_]()
...
//        }
//        deferredTask( std::packaged_task<void()>(
//             [device = vkDevice_, image = tex->vkImage_, memory0 = tex->vkMemory_[0], memory1 = tex->vkMemory_[1],
...
//         *texture, VkOffset3D{ range.offset.x, range.offset.y, range.offset.z },
...
//         },
//         texture->vkImageFormat_, outData );
//
//    return Result();
//}
//
//lvk::Result lvk::VulkanContext::upload( lvk::TextureHandle handle, const TextureRangeDesc& range,
//                                        const void* data )
//{
//    LVK_PROFILER_FUNCTION();
//
...
//             *texture, VkOffset3D{ range.offset.x, range.offset.y, range.offset.z },
//             VkExtent3D{ range.dimensions.width, range.dimensions.height, range.dimensions.depth }, vkFormat,
//             data );
//    }
//    else
//    {
...
//        };
//        stagingDevice_->imageData2D( *texture, imageRegion, range.mipLevel, range.numMipLevels, range.layer,
//                                     range.numLayers, vkFormat, data );
//    }
//
//    return Result();
//}
...
//}
//
//lvk::Holder<lvk::ShaderModuleHandle> lvk::VulkanContext::createShaderModule( const ShaderModuleDesc& desc,
//                                                                             Result*                 outResult )
//{
//    Result            result;
...
//}
//
//lvk::ShaderModuleState lvk::VulkanContext::createShaderModuleFromSPIRV( const void* spirv, size_t numBytes,
//                                                                        const char* debugName,
...
//
//    VK_ASSERT(
//         lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_SHADER_MODULE, (uint64_t)vkShaderModule, debugName ) );
//
...
//}
//
//lvk::ShaderModuleState lvk::VulkanContext::createShaderModuleFromGLSL( ShaderStage stage, const char* source,
//                                                                       const char* debugName,
//                                                                       Result*     outResult ) const
//{
...
//}
//
//bool lvk::VulkanContext::getQueryPoolResults( QueryPoolHandle pool, uint32_t firstQuery, uint32_t queryCount,
//                                              size_t dataSize, void* outData, size_t stride ) const
//{
//    VkQueryPool vkPool = *queriesPool_.get( pool );
//
//    VK_ASSERT( vkGetQueryPoolResults( vkDevice_, vkPool, firstQuery, queryCount, dataSize, outData, stride,
...
//                VK_ASSERT(
//                     vkEnumerateInstanceExtensionProperties( layer, &count, allInstanceExtensions.data() + sz ) );
//            }
//        }
//    }
//
...
//              config_.enableValidation ? (uint32_t)LVK_ARRAY_NUM_ELEMENTS( validationFeaturesEnabled ) : 0u,
//         .pEnabledValidationFeatures = config_.enableValidation ? validationFeaturesEnabled : nullptr,
//#endif
//#if defined( __APPLE__ )
//         .disabledValidationFeatureCount =
//              config_.enableValidation ? (uint32_t)LVK_ARRAY_NUM_ELEMENTS( validationFeaturesDisabled ) : 0u,
...
//    const VkBool32 gpuav_descriptor_checks =
//         VK_FALSE; // https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/8688
//    const VkBool32 gpuav_indirect_draws_buffers =
//         VK_FALSE; // https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/8579
//    const VkBool32 gpuav_post_process_descriptor_indexing =
//         VK_FALSE; // https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/9222
//#define LAYER_SETTINGS_BOOL32( name, var )                                                                        \
//    VkLayerSettingEXT                                                                                             \
//    {                                                                                                             \
//        .pLayerName = kDefaultValidationLayers[0], .pSettingName = name,                                          \
//        .type = VK_LAYER_SETTING_TYPE_BOOL32_EXT, .valueCount = 1, .pValues = var,                                \
//    }
...
//         .enabledLayerCount =
//              config_.enableValidation ? (uint32_t)LVK_ARRAY_NUM_ELEMENTS( kDefaultValidationLayers ) : 0u,
...
//#elif defined( VK_USE_PLATFORM_ANDROID_KHR )
//    const VkAndroidSurfaceCreateInfoKHR ci = { .sType  = VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR,
//                                               .pNext  = nullptr,
//                                               .flags  = 0,
...
//}
//
//uint32_t lvk::VulkanContext::queryDevices( HWDeviceType deviceType, HWDeviceDesc* outDevices,
...
//            strncpy( outDevices[numCompatibleDevices].name, deviceProperties.deviceName,
...
//    if ( desc.guid == 0UL )
//    {
//        LOG_WARN( "Invalid hardwareGuid(%lu)", desc.guid );
...
//    {
//        LOG_WARN( "VK_QUEUE_GRAPHICS_BIT is not supported" );
...
//    {
//        LOG_WARN( "VK_QUEUE_COMPUTE_BIT is not supported" );
...
//              vkFeatures10_.features.vertexPipelineStoresAndAtomics, // enable if supported
...
//              vkFeatures12_.uniformAndStorageBuffer8BitAccess,                        // enable if supported
...
//        return true;
//    };
//    auto addOptionalExtensions =
//         [&allDeviceExtensions, &deviceExtensionNames,
//          &createInfoNext]( const char* name1, const char* name2, bool& enabled, void* features = nullptr ) mutable
//    {
...
//                           VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME, hasAccelerationStructure_,
//                           &accelerationStructureFeatures );
//    addOptionalExtension( VK_KHR_RAY_QUERY_EXTENSION_NAME, hasRayQuery_, &rayQueryFeatures );
//    addOptionalExtension( VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME, hasRayTracingPipeline_,
//                          &rayTracingFeatures );
//    addOptionalExtension( VK_EXT_SWAPCHAIN_MAINTENANCE_1_EXTENSION_NAME, has_EXT_swapchain_maintenance1_,
//                          &swapchainMaintenance1Features );
//#if defined( VK_KHR_INDEX_TYPE_UINT8_EXTENSION_NAME )
//    if ( !addOptionalExtension( VK_KHR_INDEX_TYPE_UINT8_EXTENSION_NAME, has8BitIndices_,
...
//        {
//            LOG_CRITICAL( "Missing Vulkan device extensions: %s\n", missingExtensions.c_str() );
//            assert( false );
//            return Result( Result::Code::RuntimeError );
//        }
//    }
...
//
//    VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_DEVICE, (uint64_t)vkDevice_,
//                                        "Device: VulkanContext::vkDevice_" ) );
//
...
//    }();
//
//    immediate_ = std::make_unique<lvk::VulkanImmediateCommands>( vkDevice_, deviceQueues_.graphicsQueueFamilyIndex,
...
//    {
//        pimpl_->vma_ =
//             lvk::createVmaAllocator( vkPhysicalDevice_, vkDevice_, vkInstance_,
...
//         },
//         nullptr, Format_Invalid, "Sampler: default" );
//
...
//        VK_ASSERT( vkGetPhysicalDeviceCalibrateableTimeDomainsEXT( vkPhysicalDevice_, &numTimeDomains,
//                                                                   timeDomains.data() ) );
//    }
//
...
//    if ( hasHostQuery )
//    {
//        pimpl_->tracyVkCtx_ = TracyVkContextHostCalibrated( vkPhysicalDevice_, vkDevice_, vkResetQueryPool,
...
//        VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_COMMAND_POOL,
//                                            (uint64_t)pimpl_->tracyCommandPool_,
...
//        if ( hasCalibratedTimestamps_ )
//        {
//            pimpl_->tracyVkCtx_ = TracyVkContextCalibrated(
//                 vkPhysicalDevice_, vkDevice_, deviceQueues_.graphicsQueue, pimpl_->tracyCommandBuffer_,
//                 vkGetPhysicalDeviceCalibrateableTimeDomainsEXT, vkGetCalibratedTimestampsEXT );
//        }
//        else
//        {
//            pimpl_->tracyVkCtx_ = TracyVkContext( vkPhysicalDevice_, vkDevice_, deviceQueues_.graphicsQueue,
...
//    {
//        LOG_WARN( "Call initContext() first" );
...
//
//    timelineSemaphore_ = lvk::createSemaphoreTimeline( vkDevice_, swapchain_->getNumSwapchainImages() - 1,
...
//
//#if LVK_VULKAN_PRINT_COMMANDS
//    LOG_INFO( "growDescriptorPool(%u, %u)\n", maxTextures, maxSamplers );
//#endif // LVK_VULKAN_PRINT_COMMANDS
//
//    if ( !LVK_VERIFY( maxTextures <=
//                      vkPhysicalDeviceVulkan12Properties_.maxDescriptorSetUpdateAfterBindSampledImages ) )
//    {
//        LOG_WARN( "Max Textures exceeded: %u (max %u)", maxTextures,
//                  vkPhysicalDeviceVulkan12Properties_.maxDescriptorSetUpdateAfterBindSampledImages );
//    }
//
//    if ( !LVK_VERIFY( maxSamplers <=
//                      vkPhysicalDeviceVulkan12Properties_.maxDescriptorSetUpdateAfterBindSamplers ) )
//    {
//        LOG_WARN( "Max Samplers exceeded %u (max %u)", maxSamplers,
//                  vkPhysicalDeviceVulkan12Properties_.maxDescriptorSetUpdateAfterBindSamplers );
//    }
//
...
//        hasYUVImages =
//             isTextureAvailable && img->isSampledImage() && lvk::getNumImagePlanes( img->vkImageFormat_ ) > 1;
//        if ( hasYUVImages )
//        {
//            break;
//        }
...
//            // multisampled images cannot be directly accessed from shaders
//            const bool isTextureAvailable = ( img->vkSamples_ & VK_SAMPLE_COUNT_1_BIT ) == VK_SAMPLE_COUNT_1_BIT;
//            const bool isYUVImage =
//                 isTextureAvailable && img->isSampledImage() && lvk::getNumImagePlanes( img->vkImageFormat_ ) > 1;
//            immutableSamplers.push_back(
//                 isYUVImage ? getOrCreateYcbcrSampler( vkFormatToFormat( img->vkImageFormat_ ) ) : dummySampler );
//        }
...
//         lvk::getDSLBinding( kBinding_YUVImages, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
//                             (uint32_t)immutableSamplers.size(), stageFlags, immutableSamplersData ),
//         lvk::getDSLBinding( kBinding_AccelerationStructures, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR,
//                             maxAccelStructs, stageFlags ),
//    };
...
//    VK_ASSERT( lvk::setDebugObjectName( vkDevice_, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, (uint64_t)vkDSL_,
//                                        "Descriptor Set Layout: VulkanContext::vkDSL_" ) );
//
//    {
...
//             .maxSets = 1,
//             .poolSizeCount =
//                  uint32_t( hasAccelerationStructure_ ? kBinding_NumBindings : kBinding_NumBindings - 1 ),
...
//}
//
//lvk::BufferHandle lvk::VulkanContext::createBuffer( VkDeviceSize bufferSize, VkBufferUsageFlags usageFlags,
//                                                    VkMemoryPropertyFlags memFlags, lvk::Result* outResult,
//                                                    const char* debugName )
//{
//    LVK_PROFILER_FUNCTION_COLOR( LVK_PROFILER_COLOR_CREATE );
//
...
//
//        vmaCreateBufferWithAlignment( (VmaAllocator)getVmaAllocator(), &ci, &vmaAllocInfo, 16, &buf.vkBuffer_,
//                                      &buf.vmaAllocation_, nullptr );
//
//        // handle memory-mapped buffers
//        if ( memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT )
//        {
...
//
//            VK_ASSERT(
//                 lvk::allocateMemory( vkPhysicalDevice_, vkDevice_, &requirements, memFlags, &buf.vkMemory_ ) );
...
//}
//
//void lvk::VulkanContext::bindDefaultDescriptorSets( VkCommandBuffer cmdBuf, VkPipelineBindPoint bindPoint,
//                                                    VkPipelineLayout layout ) const
//{
//    LVK_PROFILER_FUNCTION();
//    const VkDescriptorSet dsets[4] = { vkDSet_, vkDSet_, vkDSet_, vkDSet_ };
//    vkCmdBindDescriptorSets( cmdBuf, bindPoint, layout, 0, (uint32_t)LVK_ARRAY_NUM_ELEMENTS( dsets ), dsets, 0,
//                             nullptr );
//}
//
...
//         awaitingNewImmutableSamplers_ || newMaxAccelStructs != currentMaxAccelStructs_ )
//    {
...
//}
//
//lvk::SamplerHandle lvk::VulkanContext::createSampler( const VkSamplerCreateInfo& ci, lvk::Result* outResult,
//                                                      lvk::Format yuvFormat, const char* debugName )
//{
//    LVK_PROFILER_FUNCTION_COLOR( LVK_PROFILER_COLOR_CREATE );
//
...
//        vkGetPhysicalDeviceSurfaceFormatsKHR( vkPhysicalDevice_, vkSurface_, &formatCount,
//                                              deviceSurfaceFormats_.data() );
//    }
//
...
//        vkGetPhysicalDeviceSurfacePresentModesKHR( vkPhysicalDevice_, vkSurface_, &presentModeCount,
//                                                   devicePresentModes_.data() );
//    }
//}
//
...
//{
//    while ( !pimpl_->deferredTasks_.empty() &&
//            immediate_->isReady( pimpl_->deferredTasks_.front().handle_, true ) )
//    {
...
//}
//
//void lvk::VulkanContext::invokeShaderModuleErrorCallback( int line, int col, const char* debugName,
//                                                          VkShaderModule sm )
//{
