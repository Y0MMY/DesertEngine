...
namespace Desert::Graphic
{
    ...
    // indexed by a push constant, while DataDrivenMaterial writes a per-material uniform block generated by
    // the DSL's `Properties Binding(n)`.
    //
    // WHICH ONE WINS IS NO LONGER AN OPINION. Measured 2026-09-04 in Debug, reading the mesh pass's own
    ...
    //
    // And the correctness argument points the same way, which is the stronger half. A per-material block
    // IS the parameters, so one material holds exactly one set of values, and
    ...
    // is the graph parameter `Blend` (0.0 / 0.5 / 1.0) and all three render RED, the Blend = 0 colour;
    // MAT_ProbeSharedBlockSingle.desce is the same entity alone at Blend = 1.0 and renders BLUE. So the
    // override path works and the sharing is what breaks it — the same defect shape as the bone matrices
    ...
    //
    // So the surviving transport is the `Materials[]` buffer and the per-material block is what gets
    // deleted. That is not done here because it is not in one place: the DSL has to emit a storage block
    // plus a MaterialIndex instead of `uniform MaterialUB`
    // (Engine/Core/ShaderCompiler/DShader/DShaderParser.cpp, BuildAutoDeclarations), DataDrivenMaterial has
    // to pack rows instead of writing fields, MeshRenderer::DrawGenericMeshes has to group and index, and
    // the terrain and the SDF text queue ride the same block. The census that will notice when it lands is
    // Desert/Tests/Engine/ShippedShaderPasses, TheParameterTransportCensusIsWhatItWasLeftAt.
    //
