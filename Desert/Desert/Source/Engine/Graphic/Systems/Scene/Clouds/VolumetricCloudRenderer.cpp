    ...
    void VolumetricCloudRenderer::DispatchWeather()
    {
        ...
        for ( uint32_t i = 0; i < m_Layers.Count; ++i )
        {
            ...
            LOG_WARN( "[Clouds] Layer {}: Weather Tile Size {:.1f} km over a layer at {:.2f}-{:.2f} km: a "
                      "ground camera sees {:.1f} coverage cells across the sky above 20 degrees, not the "
                      "{:.1f} a sky needs to read as cloud, gap and cloud with room to spare. The layer's "
                      "altitude asks for {:.1f} km.",
