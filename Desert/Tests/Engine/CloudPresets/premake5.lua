...
project(test_name)
    ...
    -- The units under test (Graphic/CloudPresets.hpp, Graphic/CloudQuality.hpp) are header-only pure
    -- functions over the component struct, so there is no renderer and no GPU to link. The generated
    -- reflection table is compiled straight in, because its registrations are static initializers.
    files {
        test_files,
