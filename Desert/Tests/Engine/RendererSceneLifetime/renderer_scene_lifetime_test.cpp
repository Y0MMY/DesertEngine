...
// these".
//
// Measured on Clouds_Protocol, one session, this machine shared with other agents: the pre-split Init cost
// 141-232 ms per load (min 141, N = 14) and the rebind costs 0.4-0.5 ms (min 0.4, N = 12). The number that
// matters to a person is bigger than that and is the reason this suite exists at all: the whole load, from
// the command to the frame that shows it, went from 6817-7747 ms to 90-537 ms, because destroying the
...
// whose own staleness test was already correct".
//
