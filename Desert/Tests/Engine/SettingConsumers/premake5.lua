...
project(test_name)
    ...
    -- test enumerates that table, so without this the parallel build can compile a stale copy and audit
    -- yesterday's field set. Build-order only; nothing is linked from it.
    dependson { "Desert" }

    -- Nothing is linked from the engine: the audit reads the consumer sources as TEXT, which is what lets
    -- it check a render pass without a GPU. The generated reflection table is compiled straight in,
    -- because its registrations are static initializers.
    files {
        test_files,
